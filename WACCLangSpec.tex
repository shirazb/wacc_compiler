...
\begin{table}
  ...
  \begin{tabulary}{\textwidth}{C|L|C|C}
    ...
    \hline
    \lit*{string} or \lit*{char[]} & Strings and char[] arrays are treated the same in WACC. Printing them prints a string 
     & \lit*{"string"} & \lit*{"string"} \\
    \hline
    Other Array Types & Outputs the address of the array & \lit*{[1, 2, 3]\footnotemark[1]} & "0x213" \\  
    \hline
    \lit*{pair} & Outputs the address of the pair & \lit*{newpair(a, b)\footnotemark[1]} & "0x232" \\
    \hline
  \end{tabulary}
...
A while loop repeatedly evaluates a boolean expression and executes a statement, continuing to do so as long as the expression evaluates to true. A while loop consists of the keyword  \lit{while} followed by the expression.  The statement is enclosed within the  \lit{do} and  \lit{end} keywords. If the boolean expression evaluates to false, the program resumes execution after the \lit{end} keyword. Thus, if the condition is initially false, the statement is not executed at all.
The statement introduces a new child scope, in which the regular scoping rules apply. On each iteration, this scope is recreated, meaning any variables declared within the statement are destroyed and then redeclared.
...
The semi colon is an infix operator which is used to execute one statement after another. 
...
Side-effect free expressions are ones which evaluate to a value and have no effect on the state of the program. So you cant do things
like change the value of a variable, affect some external state e.g write to a file , or change something about the state of the program.
...
\begin{table}
  ...
  \begin{tabular}{cclc}
    ...
    \lit*{\char`\\n} & \lit*{0x0a} & newline & \lit*{LF} or \lit*{NL} \\
    \lit*{\char`\\f} & \lit*{0x0c} & formfeed & \lit*{FF} or \lit*{NP} \\
...
\begin{table}
  ...
  \begin{tabulary}{\textwidth}{CCCL}
    ...
    \lit{ord} & char & int & Return ascii value of argument \\
    \lit{chr} & int & char & Returns corresponding ascii char of the given integer value \\
    \hline
  \end{tabulary}
...
The 'ord' operator takes as input a char and returns the corresponding numerical representation of that char as defined
in the ASCII table.

\item The \lit{chr} operator ... \fillgap{Define/describe the \lit{chr} operator}{1 mark} 
The 'chr' operator takes as input a integer and returns the corresponding character as defined in the ASCII table.
\end{itemize}
...
\begin{table}
  ...
  \begin{tabulary}{\textwidth}{CCCCCL}
    ...
    \lit{>} & 3 & int or char & int or char & bool & Greater Than \\
    \lit{>=} & 3 & int or char & int or char & bool & Greater Than or Equal \\
    \lit{<} & 3 & int or char & int or char & bool & Less Than \\
    \lit{<=} & 3 & int or char & int or char & bool & Less Than or Equal \\
    \lit{==} & 4 & base types(string?) and array and pair & base types(string?) and array and pair & bool & Equality \\
    \lit{!=} & 4 & base types(string?) and array and pair & base types(string?) and array and pair & bool & Inequality \\
...
The wacc language currently supports single line comments, comments start with a "\#"  followed by any character except new line (EOL), the first newline char marks the end of the comment. WACC does not support multi line comments, comments can be inlined and can appear anywhere within the program, even before and after the program body. Comments are ignored by the compiler and they are discarded in the lexical analysis stage along with whitespaces. The purpose of comments is to aid humans reading the source code and they bear no semantic meaning when it comes to executing the code hence they are meaningless to the compiler.

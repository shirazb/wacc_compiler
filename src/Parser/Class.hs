...
-- POST: Parses a class definition
...
parseClass = do
  ...
  fields      <- many $ parseField <* require (punctuation ';')
                "Missing \';\' denoting end of field declaration"
  constructor <- require parseConstructor ("Class " ++ pretty className ++
                 " has no constructor. \nPlease provide an \n\
                 \    init(<param-list>) is \n  <stat>\n    end.")
...
-- POST: Parses a field definition
...
-- POST: Parses a constructor definition
...
parseConstructor = do
  ...
  require (keyword "is") "Missing \'is\' keyword denoting start of constructor\
                         \ body"
  body     <- require parseStatement "Invalid constructor body"
  require (keyword "end") "Missing \'end\' keyword denoting end of constructor\
                         \ body"
